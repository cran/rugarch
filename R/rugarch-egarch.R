		...
		fit.control = list(stationarity = 1, fixed.se = 0, scale = 0))
{
	...
	if(use.solver){
		...
		solution = .garchsolver(solver, pars = ipars[estidx, 1], fun = .egarchLLH, Ifn, ILB, IUB, 
	...
	if(convergence == 0){
		...
		fit = .makefitmodel(garchmodel = "eGARCH", f = .egarchLLH, data = data,  T = T, m = m, timer = timer, 
...
# SECTION eGARCH LLH
#---------------------------------------------------------------------------------
.egarchLLH = function(pars, data, returnType = "llh", garchenv)
{
	# prepare inputs
	...
	ans = try( .C("egarchfilterC", model = as.integer(modelinc), pars = as.double(ipars[,1]), idx = as.integer(idx[,1]-1), hEst = as.double(hEst), 
					...
					h = double(T), z = double(T), llh = double(1), LHT = double(T), PACKAGE = "rugarch"), silent = TRUE )
...
#---------------------------------------------------------------------------------
.egarchfilter = function(spec, data, out.sample = 0, n.old = NULL)
{
		...
		external.forecasts = list(mregfor = NULL, vregfor = NULL), ...)
{
	...
	flt = .egarchfilter(data = tmp[,1,drop=FALSE], spec = fspec, n.old = N)
		...
		mexsimdata = NULL, vexsimdata = NULL)
{
	...
	for(i in 1:m.sim){
		...
		ans1 = try(.C("egarchsimC", model = as.integer(modelinc), pars = as.double(ipars[,1]), idx = as.integer(idx[,1]-1), 
						meanz = as.double(kappa), h = as.double(h), z = as.double(z[,i]), res = as.double(res),
		...
		mexsimdata = NULL, vexsimdata = NULL)
{
	...
	ans = .Call("megarchsim", model = as.integer(modelinc), pars = as.numeric(ipars[,1]), idx = as.integer(idx[,1]-1), 
			meanz = as.numeric(kappa), h = h, z = z, res = res, vxs = vxs, N = as.integer( c(m, n) ), PACKAGE = "rugarch", DUP = FALSE)
		...
		vexsimdata = NULL)
{
	...
	ans = .Call("megarchsim", model = as.integer(modelinc), pars = as.numeric(ipars[,1]), idx = as.integer(idx[,1]-1), 
			meanz = as.numeric(kappa), h = h, z = z, res = res, vxs = vxs, N = as.integer( c(m, n) ), PACKAGE = "rugarch")
