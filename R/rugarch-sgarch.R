...
object instead\n")
	...
	if(use.solver){
		...
		solution = .garchsolver(solver, pars = ipars[estidx, 1], fun = .sgarchLLH, Ifn, ILB, IUB, 
	...
	if(convergence == 0){
		...
		fit = .makefitmodel(garchmodel = "sGARCH", f = .sgarchLLH, data = data,  T = T, m = m, timer = timer, 
		...
		external.forecasts = list(mregfor = NULL, vregfor = NULL), ...)
{
	...
	flt = .sgarchfilter(data = tmp[,1,drop=FALSE], spec = fspec, n.old = N)
