		...
		fit.control = list(stationarity = 1, fixed.se = 0, scale = 0))
{
	...
	if(use.solver){
		...
		solution = .garchsolver(solver, pars = ipars[estidx, 1], fun = .aparchLLH, Ifn, ILB, IUB, 
	...
	if(convergence == 0){
		...
		fit = .makefitmodel(garchmodel = "apARCH", f = .aparchLLH, data = data,  T = T, m = m, timer = timer, 
...
.aparchLLH = function(pars, data, returnType = "llh", garchenv)
{
	...
	hEst = (mean(abs(res)^ipars[idx["delta",1],1], na.rm = TRUE))^(1/ipars[idx["delta",1],1])
	if(modelinc[15]>0) {
	...
	if(fit.control$stationarity == 1 && modelinc[15] == 0){
		if(!is.na(persist) && persist >= 1) return(llh = get(".llh", garchenv) + 0.1*(abs(get(".llh", garchenv))))
	}
	...
	ans = try( .C("aparchfilterC", model = as.integer(modelinc), pars = as.double(ipars[,1]), idx = as.integer(idx[,1]-1), hEst = as.double(hEst), 
					x = as.double(data), res = as.double(res), e = double(T), mexdata = mexdata, vexdata = vexdata, zrf = as.double(zrf),
					constm = double(T), condm = double(T), m = as.integer(m), T = as.integer(T),
					h = double(T), z = double(T), llh = double(1), LHT = double(T), PACKAGE = "rugarch"),
			silent = TRUE)
...
.aparchfilter = function(spec, data, out.sample = 0, n.old = NULL)
{
	...
	hEst = (mean(mvar^ipars[idx["delta",1],1], na.rm = TRUE))^(1/ipars[idx["delta",1],1])
	...
	ans = try( .C("aparchfilterC", model = as.integer(modelinc), pars = as.double(ipars[,1]), 
					...
					vexdata = vexdata, zrf = as.double(zrf), constm = double(T), condm = double(T), 
					m = as.integer(m), T = as.integer(T), h = double(T), z = double(T), llh = double(1), 
		...
		external.forecasts = list(mregfor = NULL, vregfor = NULL), ...)
{
	...
	flt = .aparchfilter(data = tmp[,1,drop=FALSE], spec = fspec, n.old = N)
		...
		mexsimdata = NULL, vexsimdata = NULL)
{
	...
	if(is.na(presigma[1])){
		if(startMethod[1] == "unconditional"){
			kappa = 1
			hEst = (ipars[idx["omega",1],1]/abs(1-persist))^(1/ipars[idx["delta",1],1])
	...
	for(i in 1:m.sim){
		...
		ans1 = try(.C("aparchsimC", model = as.integer(modelinc), pars = as.double(ipars[,1]), idx = as.integer(idx[,1]-1), 
						h = as.double(h), z = as.double(z[,i]), res = as.double(res), vexdata = as.double(vexsim[[i]]), 
						T = as.integer(n+m), m = as.integer(m), PACKAGE = "rugarch"), silent = TRUE)
		...
		mexsimdata = NULL, vexsimdata = NULL)
{
	...
	ans = .Call("maparchsim", model = as.integer(modelinc), pars = as.numeric(ipars[,1]), idx = as.integer(idx[,1]-1), 
			h = h, z = z, res = res, vxs = vxs, N = as.integer( c(m, n) ), PACKAGE = "rugarch", DUP = FALSE)
