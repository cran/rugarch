...
rugarch.test12a = function(cluster=NULL)
{
	...
	cat("\nSeries Forecasts:\n")
	head(ser<-as.data.frame(bootp, which = "series", type = "raw"))
	cat("\nSeries Summary:\n")
	as.data.frame(bootp, which = "series", type = "summary")
	cat("\nQuantile Summary:\n")
	as.data.frame(bootp, which = "series", type = "q", qtile= c(0.25, 0.75))
	sink(type="message")
	sink()
	close(zz)
	...
	# n.bootpred by n.ahead matrix:
	cat("\nSigma Forecasts:\n")
	head(sig<-as.data.frame(bootp, which = "sigma", type = "raw"))
	cat("\nSigma Summary:\n")
	as.data.frame(bootp, which = "sigma", type = "summary")
	sink(type="message")
	sink()
	close(zz)
