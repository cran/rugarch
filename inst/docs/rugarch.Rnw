...
options(keep.source = FALSE, width = 80)
foo <- packageDescription("rugarch")
setCacheDir = "D:/SweaveCache/"
@
\title{rugarch: Univariate GARCH Models.\\
(Version \Sexpr{foo$Version})}
...
\begin{figure}[!ht]
\centering
\subfloat[$\lambda=1$(HYP)]{\label{fig:ghyp2}\includegraphics[scale=0.4, angle=0]{./figs/ghyp2.png}}
\subfloat[$\lambda=-0.5$(NIG)]{\label{fig:ghyp3}\includegraphics[scale=0.4, angle=0]{./figs/ghyp3.png}}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=22cm]{./figs/fitplot.png}
...
setfixed(spec)<-list(mu=0.01, ma1=0.2, ar1=0.5, omega=1e-5,alpha1=0.03, 
		beta1=0.9, gamma1=0.01, delta=1,shape=5)
...
roll = ugarchroll(spec,  data = sp500ret, n.ahead = 1, forecast.length = 500,
		...
		parallel = TRUE, parallel.control = list(pkg="snowfall", cores=10),
		solver = "solnp", solver.control = list(tol = 1e-5, delta = 1e-6, 
				trace=0), calculate.VaR = TRUE, VaR.alpha = c(0.01, 0.05))
report(roll, type = "VaR", n.ahead = 1, VaR.alpha = 0.01, conf.level = 0.95)
report(roll, type = "fpm")
@
\begin{figure}
\centering
\includegraphics{./figs/roll.png}
...
print(persistence(pars=unlist(list(mu=0.001, ar1=0.4, ma1=-0.1, omega=1e-6, 
								alpha1=0.05, beta1=0.90, gamma1 = 0.05, 
								shape=1.5)), 
				distribution="ged", model="gjrGARCH"))

setfixed(spec)<-list(mu=0.001, ar1=0.4, ma1=-0.1, omega=1e-6, alpha1=0.05, 
		beta1=0.90, gamma1 = 0.05, shape=1.5)

dist = ugarchdistribution(fitORspec = spec, n.sim = 2000, n.start = 1, 
		m.sim = 100,  recursive = TRUE, recursive.length = 6000, 
		recursive.window = 1000, rseed = 1066,
		solver = "solnp", solver.control = list(trace=0), parallel = TRUE, 
		parallel.control = list(pkg = "snowfall", cores = 20))
...
\begin{figure}[!ht]
\centering
\includegraphics[width=22cm]{./figs/dist1.png}
\caption[GARCH Simulated Parameters Density]{Simulated Parameter Density}\label{fig:dist1}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=22cm]{./figs/dist11.png}
\caption[GARCH Parameters RMSE Rate of Change]{RMSE Rate of Change}\label{fig:dist2}
\end{figure}
\begin{figure}[!ht]
\centering
\subfloat[Bivariate Parameter Plots]{\label{fig:dist3}\includegraphics[scale=0.4, angle=0]{./figs/dist2.png}}
\subfloat[GARCH Stat Plots]{\label{fig:dist4}\includegraphics[scale=0.4, angle=0]{./figs/dist3.png}}
...
print(system.time(multifit(multispec = mspec, data = dji30ret[,1:30],parallel=TRUE,
						parallel.control = list(pkg="snowfall",cores=20))))
print(system.time(multifit(multispec = mspec, data = dji30ret[,1:30],parallel=TRUE,
				parallel.control = list(pkg="snowfall",cores=5))))
print(system.time(multifit(multispec = mspec, data = dji30ret[,1:30], 
						solver.control = list(trace=0), parallel=FALSE,
